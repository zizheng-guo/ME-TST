...
def main(config):
    ...
    if flow_process:
        ...
        final_dataset_spotting = feature_extraction_spotting(dataset_name, final_images, k, method_type)
        ...
        if dataset_name == "CASME_3":
            ...
            for i in range(5):
                if method_type == 1:
                    final_dataset_spotting.extend(pickle.load( open("cache/ME-TST+/"+dataset_name + "_dataset_" + str(i+1) +".pkl", "rb" ) )) # CASME_3 is too large and is divided into five parts
                else:
                    final_dataset_spotting.extend(pickle.load( open("cache/ME-TST/"+dataset_name + "_dataset_" + str(i+1) +".pkl", "rb" ) )) 
        else:
            if method_type == 1:
                final_dataset_spotting = pickle.load( open("cache/ME-TST+/"+dataset_name + "_dataset.pkl", "rb" ) )
            else:
                final_dataset_spotting = pickle.load( open("cache/ME-TST/"+dataset_name + "_dataset.pkl", "rb" ) )
    ...
    # Training & Evaluation
    print('\n ------ Training ------')
    TP_spot, FP_spot, FN_spot, metric_final, pred_list, gt_tp_list, TP_spot_neutral, FP_spot_neutral, FN_spot_neutral, pred_neutral_list, gt_tp_neutral_list = train_model(train, X_spot, Y_spot, Y1_spot, groupsLabel_spot, groupsLabel_recog, final_dataset_spotting, final_subjects, final_samples, final_videos, final_emotions, emotion_type, epochs, lr, batch_size, dataset_name, k_p, k, ratio, frame_skip, strategy, note, method_type)
