...
def feature_extraction_spotting(dataset_name, final_images, k, method_type):
    ...
    for video in range(len(final_images)):
        ...
        for img_count in range(final_images[video].shape[0] // (k//2) -1):
            final_image = flowProcess(detector, predictor, final_images[video], img_count * (k//2), k, True, dataset_name, 320, 1, method_type)
...
def flowProcess(detector, predictor, img, start, interval, align, dataset_name, img_size, fs, method_type):
    flow = []
    flow1_total = [[0, 0]] 
    flow1_total1 = [[0, 0]]
    flow1_total2 = [[0, 0]]
